...
- **列幅変更** — テーブルの各列の幅をドラッグで自由にリサイズ
- **列順序変更** — テーブルのヘッダーをドラッグ&ドロップで列の並び順を変更
- **6メーターダッシュボード** — Eco / Free Quota / Today / 7 Days / Billing Cycle / Forecast
- **メモ機能** — 各利用レコードにメモを追加・編集（日本語 IME 対応）
- **手動更新** — ステータスバークリック、またはテーブル画面右下の更新ボタンでデータ取得
...
ステータスバーには直近リクエストの情報がリアルタイムで表示されます。クリックでデータを手動更新できます。また、詳細テーブル画面の右下にある更新ボタンからも手動更新が可能です。
...
コマンドパレットから `Cursor Economizer: 詳細を開く` を実行すると、利用イベントの詳細テーブルが開きます。

### テーブル操作

| 操作 | 方法 |
|---|---|
| フィルタ | 各列ヘッダーの入力欄にテキストまたは数値範囲を入力 |
| ソート | 列ヘッダーをクリック |
| ページ切替 | テーブル下部のページネーション |
| 列幅変更 | 列ヘッダーの境界線をドラッグしてリサイズ |
| 列順序変更 | 列ヘッダーをドラッグ&ドロップで並べ替え |
| メモ編集 | MEMO 列をクリックしてインライン編集（Enter で保存、Escape でキャンセル） |
| データ更新 | テーブル画面右下の更新ボタンをクリック |
